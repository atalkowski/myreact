...
function toggleRefForId(type, content, htmId, button) {
 //   var ext = getFileExtension(file);
    var htm = document.getElementById(htmId);
    var label = htmId + "-label";
    if (htm.innerHTML.length > 0) {
      clearInnerHtml(htmId);
      resetButton(button, "View");
    } else {
      if (type === "file") {      
        loadTextFileToElement(content, htmId);
      } else {
...
function toggleTextFile(file, name, button) {
    toggleRefForId("file", file, name, button)
//   var ext = getFileExtension(file);
